...
static inline void process_setup_packet(uint16_t request, uint16_t value, uint16_t index, uint16_t length)
{
...
static void handle_control_transfer (uint8_t status)
{
    ...
    switch (BDT_DESC_PID(entry->desc)) {
    case BDT_PID_SETUP:
        /* We should be in the setup phase but we could also be in the data in stage if the. */
        ...
        if (phase == PHASE_DATA_IN ||
            ...
            memcpy(inputs[CONTROL_ENDPOINT].data, entry->buffer,
                   inputs[CONTROL_ENDPOINT].length < n ? inputs[CONTROL_ENDPOINT].length : n);
...
static void utostr(uint64_t n, int radix, int width)
{
...

    {
        ...
        for (; n > 0 ; *(p -= 1) = (n % radix) + (n % radix < 10 ? '0' : 'a' - 10), n /= radix);
        ...
        if (*c == '%') {
            ...
            for (c += 1 ; ; c += 1) {
                if (*c == '%') {
                    ...
                    if (sign) {
                        switch (size) {
                        case 3: itostr(va_arg(ap, int64_t), base, width); break;
                        default: itostr(va_arg(ap, int), base, width); break;
                        }
                    } else {
                        switch (size) { 
                        case 3: utostr(va_arg(ap, uint64_t), base, width); break;
                        case 2: utostr(va_arg(ap, uint32_t), base, width); break;
                        default: utostr(va_arg(ap, int), base, width); break;
                        }
                    }
