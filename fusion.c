...
static float gyroscope_offset_lines[3][2];

static void sensor_data_ready(int16_t *line, void *userdata)
{
...
static void fuse_sensor_data(void *userdata)
{
    FusionContext *context = (FusionContext *)userdata;
    int i;
    ...
    for (i = 0 ; i < context->available ; i += 1) {
        const float C_a[] = {1.0 / 16384, 1.0 / 8192,
                             1.0 / 4096, 1.0 / 2048};

        const float C_g[] = {1.0 / 131, 1.0 / 65.5,
                             1.0 / 32.8, 1.0 / 16.4};
        ...
        if (context->data & (1 << FUSION_RAW_ACCELERATION)) {
            ...
            for (k = 0 ; k < 3 ; k += 1) {
                context->line[j + k] += (float)l[0 + k] * C_a[ACCEL_FSR];
            }
        ...
        if (context->data & (1 << FUSION_RAW_ANGULAR_SPEED)) {
            ...
            for (k = 0 ; k < 3 ; k += 1) {
                context->line[j + k] += (float)l[4 + k] * C_g[GYRO_FSR];
            }
        ...
        if (context->data & (1 << FUSION_RAW_MAGNETIC_FIELD)) {
            ...
            for (k = 0 ; k < 3 ; k += 1) {
                context->line[j + k] += (float)l[7 + k] * 1.0 / 16384;
            }
        ...
        if (context->data & (1 << FUSION_ANGULAR_SPEED)) {
            int k;

            for (k = 0 ; k < 3 ; k += 1) {
                float *o = gyroscope_offset_lines[k];
                context->line[j + k] += (float)l[4 + k] * C_g[GYRO_FSR] -
                    (o[0] + ((float)l[3] / 340.0 + 36.53) * o[1]);
            }
                  ...
                  fusion_DataReadyCallback callback, void *userdata)
{
    ...
    context.count = context.available = 0;
    ...
    set_software_isr_callback(fuse_sensor_data, &context);
    prioritize_software_interrupt(3);

    sensors_set_callback(sensor_data_ready, &context);
    sleep_while (context.period > 0);

    sensors_set_callback(NULL, NULL);
    set_software_isr_callback(NULL, NULL);
}

void fusion_set_gyroscope_offsets(float (*lines)[2])
{
    memcpy(gyroscope_offset_lines, lines, sizeof(gyroscope_offset_lines));
}
